...
inline bool is_overlap(const string& label1,const string& label2) {
...
//	std::cout<<"1 "<<label1<<"2 "<<label2<<"trans ";
	string remain = label1;

	for(int i=0;i<pairnum1;i++){

		int first = remain.find('[');
		int second = remain.find(',');
		int three = remain.find(']');

		string num1 = remain.substr(first+1,second-first-1);
		string num2 = remain.substr(second+1,three-second-1);
...
//		std::cout<<"["<<a<<","<<b<<"]";
		if(i<pairnum1-1){
			remain = remain.substr(three+1);
		}

		result1.push_back(make_pair(a,b));
	}
...
//		std::cout<<"["<<a<<","<<b<<"]";
		...
		result2.push_back(make_pair(a,b));
	}

	for(int i=0;i<result1.size();i++){
		for(int j=0;j<result2.size();j++){
			int result1left = result1[i].first;
			int result1right = result1[i].second;
			int result2left = result2[j].first;
			int result2right = result2[j].second;

			if((result1left>=result2left&&result1left<=result2right)||(result2left>=result1left&&result2left<=result1right)){
				return true;
			}
		}
	}
...
inline bool is_IO_start_label(const string& label) {
	if (label.length() < 3)
		return false;
	return (label[0] == 'I' || label[0] == 'i') && label[1] == '-';
}

inline bool is_IO_continue_label(const string& label, const string& startlabel, int distance) {
