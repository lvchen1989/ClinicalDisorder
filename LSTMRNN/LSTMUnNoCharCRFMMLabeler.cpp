/*
 * LSTMUnNoCharCRFMMLabeler.cpp
 *
 *  Created on: 2016-4-4
 *      Author: chen_lyu
 */

#include "LSTMUnNoCharCRFMMLabeler.h"
...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
  ...
  m_pipe.readAllInstances(trainFile, trainInsts, m_options.maxInstance);
  if (devFile != "")
    m_pipe.readAllInstances(devFile, devInsts, m_options.maxInstance);
  if (testFile != "")
    m_pipe.readAllInstances(testFile, testInsts, m_options.maxInstance);
...
void Labeler::test(const string& testFile, const string& outputFile, const string& modelFile) {
  ...
  m_pipe.readAllInstances(testFile, testInsts);
